...
import { Card, Rate, Spin, Alert, Pagination, Layout } from 'antd'
...
export default class MovieList extends React.Component {
  constructor(props) {
    ...
    // 是否为第一次渲染
    //  第一次执行： componentDidMount 加载数据
    //  以后的每一次页码或电影类型切换执行： componentWillReceiveProps 加载数据
  ...
  componentWillReceiveProps(nextProps, nextState) {
    ...
    // console.log('componentWillReceiveProps 发送请求')
  ...
  fetchMovieData() {
    ...
    fetch(`/api/movie/${this.movieType}?start=${start}&count=${this.pageSize}`)
      ...
      .then(res => {
        ...
        // 第一次加载数据成功， 让 isFirstLoad 为false
  ...
  render() {
    ...
    return (
      // Layout 组件默认采用flex布局，因此，直接修改 布局方向即可
      <Layout>
